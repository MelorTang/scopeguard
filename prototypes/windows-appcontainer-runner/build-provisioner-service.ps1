...
if (-not $IsWindows) {
    throw "The Provisioner service must be built on Windows."
}
...
$source = Join-Path $PSScriptRoot "provisioner-service.cpp"
$output = Join-Path $OutputDirectory "scopeguard-provisioner-service.exe"
New-Item -ItemType Directory -Path $OutputDirectory -Force | Out-Null

$compileCommand = @(
    "call `"$devCommand`" -no_logo -arch=amd64 -host_arch=amd64",
    "cl.exe /nologo /std:c++20 /EHsc /W4 /WX /DUNICODE /D_UNICODE /Fe:`"$output`" `"$source`" advapi32.lib bcrypt.lib"
) -join " && "

& $env:ComSpec /d /s /c $compileCommand | ForEach-Object { Write-Host $_ }
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $output)) {
    throw "MSVC failed to build the Provisioner service."
}
