...
## Desktop product integration checkpoint

The selected runner is now wired into the ScopeGuard Desktop source on branch
`codex/managed-execution-desktop-slice` through the formal
`packages/managed-execution` interface. Request Approval and Auto Approve share
one bounded adapter; Full Access has a separate current-user adapter. The Agent
Host reaches Electron Main through private typed request/event/response/cancel
messages that are not exposed through Preload.

The product adapter requires an installed manifest, registered Workspace ID,
pinned service client, fixed Node runtime, durable Broker Profile intent,
parent-monitoring lifetime Broker, exact `registryRead` LPAC policy, Service ACL
cleanup, and Broker Profile deletion. It streams stdout/stderr and preserves
`effect=unknown` plus unconfirmed termination/cleanup rather than projecting a
successful result. Missing or malformed setup fails closed.
