...
Handle OpenInheritableOutputHandle(DWORD standard_handle) {
    if (g_stream_output) {
        ...
        if (source && source != INVALID_HANDLE_VALUE) {
            ...
            if (!DuplicateHandle(
                ...
                ThrowLastError("DuplicateHandle(standard output)");
            }
...
int RunInContainer(
    ...
    Handle standard_output = OpenInheritableOutputHandle(STD_OUTPUT_HANDLE);
    Handle standard_error = OpenInheritableOutputHandle(STD_ERROR_HANDLE);
...
int wmain(int argc, wchar_t** argv) {
    try {
        ...
        for (std::size_t index = 1; index < args.size(); ++index) {
            ...
            } else if (args[index] == L"--stream-output") {
                g_stream_output = true;
