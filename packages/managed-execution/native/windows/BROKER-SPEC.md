# Managed Execution Broker specification and validation history

## Status

This specification records the production seam demonstrated by the issue #14
LPAC prototype. The selected seam is now implemented by
`packages/managed-execution` and Electron Main, while the remaining installer,
signing, Windows 10, and packaged Windows 11 gates stay explicit.
